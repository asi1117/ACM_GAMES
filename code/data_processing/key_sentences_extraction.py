...
stop_words.update(['.', ',', 'I','"', "'", '?', '!', ':', ';', '(', ')', '[', ']', '{', '}','i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 'you',
                ...
                'won', 'wouldn','gear', 'vr','!', ',' ,'.' ,'?' ,'-s' ,'</s> ', 's',
            'one','https','half','saber','www','lift','...','..',])


readfile = r'F:\CHI\HMDs\Gener\'
writfile = r'F:\CHI\HMDs\Gener\total\topic\
...
with open(readfile, encoding='utf8') as csvfile:
    ...
    for row in reader:
        try:
            count += 1
            print(count)
            key = tokenize.sent_tokenize(row[0].lower())
            if key:
                for i in key:
                    if '  ' in str(i) \
